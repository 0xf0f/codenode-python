...
def get_function_signature(function: types.FunctionType):
...
def get_class_signature(cls: type):
    ...
    for child in ast.walk(node):
        if isinstance(child, ast.ClassDef):
            ...
            signature_kwargs = ','.join(
                ast.unparse(kwarg)
                for kwarg in child.keywords
            )

            all_args = signature_args + signature_kwargs
            if all_args:
                return f'({all_args})'
            else:
                return ''
    else:
        raise Exception(f'unable to find signature for class {cls}')
...
def get_function_documentation(function: types.FunctionType):
    return {
        ...
        'signature': f'def {function.__name__}{get_function_signature(function)}',
        **get_docstring_info(function.__doc__ or ''),
    }
...
class ClassDocumentation(DocumentationContent):
    def get_documentation(self):
        ...
        yield quote_block((
            line('```python'),
            line(f'class {cls.__name__}{documentation["signature"]}: ...'),
            line('```'),
            (
                line(f'{line_content}')
